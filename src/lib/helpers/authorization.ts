...
export function filterRoles(locals: App.Locals, url: URL, ...roles: Role[]): void {
	// Nếu người dùng đã đăng nhập
	if (locals.user) {
		// Trường hợp role route là UnauthorizedOnly
		if (roles.includes(Role.UnauthorizedOnly)) {
			error(403, {
		...
		if (!roles.includes(Role.All) && intersection(roles, locals.user.roles).length === 0) {
			error(401, {
