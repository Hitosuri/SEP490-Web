<script lang="ts">
	...
	const form = superForm(data.materialFilterForm, {
		SPA: true,
		id: 'single',
		invalidateAll: false,
		resetForm: false,
	...
	let pageSize = data.materialListPage.pageSize || 10;
	let currentPage = data.materialListPage.pageNumber || 1;
	let totalItems = data.materialListPage.totalRecords || 0;
	...
	let materials = data.materialListPage.data;
	let loading = false;
	...
	onMount(async () => {
		...
		const r = await fetch(endpoints.materialTypes.get, {
			method: 'POST',
			headers: {
